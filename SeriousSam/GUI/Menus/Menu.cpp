...
void MenuUpdateMouseFocus(void) {
  ...
  if (_pmgUnderCursor != NULL) {
    ...
    if (_pmgUnderCursor->mg_pmgLeft == NULL && _pmgUnderCursor->mg_pmgRight == NULL && _pmgUnderCursor->mg_pmgUp == NULL
...
#undef LOGOSIZE
...

      {
        ...
        dpMenu.PutTexture(&_toLogoMenuA,
                          PIXaabbox2D(PIX2D(pixCenterI - pixSizeI, pixHeightJ), PIX2D(pixCenterI, pixHeightJ + pixSizeJ)));
        dpMenu.PutTexture(&_toLogoMenuB,
                          PIXaabbox2D(PIX2D(pixCenterI, pixHeightJ), PIX2D(pixCenterI + pixSizeI, pixHeightJ + pixSizeJ)));
      }
...
#define THUMBH 96
    ...
    if (_bThumbnailOn) {
      ...
      if (_toThumbnail.GetData() != NULL) { // show thumbnail with shadow and border
        ...
        dpMenu.DrawBorder(pixI0, pixJ0, THUMBW * fThumbScaleW, THUMBH * fScaleH,
                          LCDGetColor(C_mdGREEN | 255, "thumbnail border"));
      } else {
        dpMenu.SetFont(_pfdDisplayFont);
        dpMenu.SetTextScaling(fScaleW);
        dpMenu.SetTextAspect(1.0f);
        dpMenu.PutTextCXY(TRANS("no thumbnail"), (pixI0 + pixI1) / 2, (pixJ0 + pixJ1) / 2,
                          LCDGetColor(C_GREEN | 255, "no thumbnail"));
      }
    }
...
extern void FixupBackButton(CGameMenu *pgm) {
  ...
  mgBack.mg_pmgLeft = mgBack.mg_pmgRight = mgBack.mg_pmgUp = mgBack.mg_pmgDown = pgm->gm_pmgSelectedByDefault;
