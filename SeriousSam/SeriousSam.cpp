...
static void DirectoryLockOn(void) {
  ...
  _hLock = CreateFileA(_fnmLock, GENERIC_WRITE, 0 /*no sharing*/,
...
void UpdatePauseState(void) {
  BOOL bShouldPause = (_gmRunningGameMode == GM_SINGLE_PLAYER)
                      && (bMenuActive || _pGame->gm_csConsoleState == CS_ON || _pGame->gm_csConsoleState == CS_TURNINGON
...
void PrintDisplayModeInfo(void) {
  // skip if timed out
  if (_pTimer->GetTimeTick() > (_llDisplayModeChanged + CTimer::InTicks(sam_tmDisplayModeReport)))
    return;
  ...
  while (_bRunning && _fnmModToLoad == "") {
    ...
    while (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
      ...
      if ((msg.message == WM_ACTIVATE && (LOWORD(msg.wParam) == WA_INACTIVE || HIWORD(msg.wParam)))
          || msg.message == WM_CANCELMODE || msg.message == WM_KILLFOCUS || (msg.message == WM_ACTIVATEAPP && !msg.wParam)) {
      ...
      // if application is activated or minimized
               ...
               || msg.message == WM_SETFOCUS || (msg.message == WM_ACTIVATEAPP && msg.wParam)) {
      ...
      BOOL bMenuForced
        = (_gmRunningGameMode == GM_NONE && (_pGame->gm_csConsoleState == CS_OFF || _pGame->gm_csConsoleState == CS_TURNINGOFF));
...
#define WM_MOUSEWHEEL 0x020A
      ...
      // if toggling console
      BOOL bConsoleKey = sam_bToggleConsole
      ...
      if (_gmRunningGameMode == GM_DEMO || _gmRunningGameMode == GM_INTRO) {
        ...
        BOOL bTilde
          = (msg.message == WM_KEYDOWN && (msg.wParam == VK_F1 || MapVirtualKey(msg.wParam, 0) == 41)); // scan code for '~'
...
BOOL TryToSetDisplayMode(enum GfxAPIType eGfxAPI, INDEX iAdapter, PIX pixSizeI, PIX pixSizeJ, enum DisplayDepth eColorDepth,
  ...
  if (bSuccess) {
    ...
    if (!bSuccess || !_pGfx->IsCurrentModeAccelerated()) { // report error
...
void StartNewMode(enum GfxAPIType eGfxAPI, INDEX iAdapter, PIX pixSizeI, PIX pixSizeJ, enum DisplayDepth eColorDepth,
                  BOOL bFullScreenMode) {
  CPrintF(TRANS("\n* START NEW DISPLAY MODE ...\n"));
  ...
  if (!bSuccess) {
    ...
    // if all failed
    if (!bSuccess) {
      FatalError(TRANS("Cannot set display mode!\n"
