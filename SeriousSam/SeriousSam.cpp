...
int SubMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
  ...
  while (_bRunning && _fnmModToLoad == "") {
    ...
    while (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
      ...
      BOOL bMenuForced = (_gmRunningGameMode == GM_NONE && (_pGame->gm_csConsoleState == CS_OFF || _pGame->gm_csConsoleState == CS_TURNINGOFF));
      ...
      if (_gmRunningGameMode == GM_DEMO || _gmRunningGameMode == GM_INTRO) {
        ...
        BOOL bTilde = (msg.message == WM_KEYDOWN && (msg.wParam == VK_F1 || MapVirtualKey(msg.wParam, 0) == 41)); // scan code for '~'
